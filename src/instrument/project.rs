...
fn inst_project_workspace(workspace_path: &Path, output_path: &Path, debugee_path: &Path, input_target_dir: &Path, inst_spec: &InstProjectSpec) -> Result<(), Box<dyn std::error::Error>> {
