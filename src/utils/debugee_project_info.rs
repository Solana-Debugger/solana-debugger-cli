...
pub struct DebugeeProjectInfo {
    ...
    pub compile_output_path: PathBuf,
    pub target_name: String,
    pub target_so_name: String,
}
...
pub fn get_program_info(program_path: &Path) -> Result<DebugeeProjectInfo, Box<dyn std::error::Error>> {
    ...
    let compile_output_path = target_directory.join("sbf-solana-solana").join("release");

    let target_name = target.name.clone();

    let target_so_name = target.name.clone() + ".so";
